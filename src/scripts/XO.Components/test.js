        ...
        attach() {
            ...
            this.addEventListener("click", e => {
                ...
                if (e.path[0].id === "xo-day") {
                    var exist = __find__(this.data, e.path[0].getAttribute("date")),
                        date = new Date(e.path[0].getAttribute("date")),
                        data = "";
                    if (exist !== null) {
                        var events = this.data[exist]["events"];
                        Object.keys(events).reverse().forEach(e => {
                            data += `<div id="row" idx="${e}">${events[e]["title"]}<button>&times;</button></div>`;
                        });
                    }
...
function __run__(el) {
    el.queryAll("#xo-day").forEach(d => {
        var exist = __find__(el.data, d.getAttribute("date"))
        if (exist !== null) {
...
function __create__(el, date) {
    el.query("#modal input").addEventListener("keydown", e => {
        if (e.keyCode === 13 && e.target.value.trim()) {
            ...
            var exist = __find__(el.data, date);
            var idx = (exist !== null) ? Object.keys(el.data[exist]["events"]).length : 0;
            if (exist === null) {
                el.data.push({
                    date: date,
                    events: {}
                });
                exist = el.data.length - 1;
            }
            el.query("#modal main section").insertAdjacentHTML("afterbegin", `
                <div id="row" idx="${idx}">${e.target.value.trim()}<button>&times;</button></div>
            `);
            el.data[exist]["events"][idx] = {
    ...
    el.queryAll("#row button").forEach(b => {
        b.addEventListener("click", () => {
            var idx = b.parentElement.getAttribute("idx"),
                exist = __find__(el.data, date);
            if (exist !== null) delete el.data[exist]["events"][idx];
            if (Object.keys(el.data[exist]["events"]).length === 0) el.data.splice(exist, 1);
...
function __find__(list, date) {
    var exist = null;
    list.forEach((e, i) => {
        if (e.date === date) exist = i;
    });
    return exist;
}